...
func updateAPIData(db *sql.DB, apiURL string, columnName string) {
	...
	for {
		...
		if nextURL, ok := data["next"].(string); ok && nextURL != "" && columnName != "previous_launches" {
			apiURL = nextURL
		} else {
			break
		}
...
func LaunchDataFetcher(db *sql.DB) {
	...
	apiMap := map[string]string{
		...
		"previous_launches": "https://ll.thespacedevs.com/2.2.0/launch/previous/?format=json&limit=100&mode=detailed",
	...
	for column := range apiMap {
		if column == "astronauts" || column == "launch_vehicles" || column == "spacecraft" {
			tickers[column] = time.NewTicker(10 * time.Minute) // Update every 60 minutes
		} else {
